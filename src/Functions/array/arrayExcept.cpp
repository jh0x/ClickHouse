...
template <bool nullable, bool exclude_is_const, typename ColumnT>
void processImpl(ConstColumnInfo<ColumnT> source, ConstColumnInfo<ColumnT> exclude, ColumnInfo<ColumnT> result, size_t input_rows_count)
{
    ...
    {
        result.col.getChars().clear();
        if constexpr (nullable)
            result.null_map->resize(source.null_map->size());
    }
    else
    {
        auto & result_data = result.col.getData();
        result_data.reserve(source.col.size());
        if constexpr (nullable)
            result.null_map->resize(source.null_map->size());
    }
...
public:
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr & return_type, size_t input_rows_count) const override
    {
        ...
        auto source_full_col = arguments[0].column->convertToFullIfNeeded();
